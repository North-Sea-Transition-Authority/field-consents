...
class FileControllerHelperServiceTest {
  ...
  private final FieldConsentsFileUsage testFileUsage = new TestFileUsage("usageId", "usageType", "documentType");
  ...
  void download() {
    ...
    Supplier<FieldConsentsFileUsage> usageSupplier = () -> testFileUsage;
  ...
  void findFileAndThen_fileExists_hasUsage_valid() {
    ...
    verify(fieldConsentsFileService).throwIfFileDoesNotBelongToUsage(uploadedFile, testFileUsage);
  }
  ...
  void findFileAndThen_fileDoesNotExist() {
    ...
    verify(fieldConsentsFileService, never()).throwIfFileDoesNotBelongToUsage(any(), any());
  }
  ...
  void findFileAndThen_fileExists_doesNotHaveUsage_doesBelongToUser() {
    ...
    when(fieldConsentsFileService.fileBelongsToUser(uploadedFile, userDetail)).thenReturn(true);
  ...
  void findFileAndThen_fileExists_doesNotHaveUsage_doesNotBelongToUser() {
    ...
    when(fileService.find(fileId)).thenReturn(Optional.of(uploadedFile));
    when(fieldConsentsFileService.fileHasUsage(uploadedFile)).thenReturn(false);
    when(fieldConsentsFileService.fileBelongsToUser(uploadedFile, userDetail)).thenReturn(false);
