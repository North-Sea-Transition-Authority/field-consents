...
class WithdrawalCaseEventServiceTest {
  ...
  @Test
  void getCaseEventViewForApplicationWithdrawal_onRequest() {
    var portalUsersDtoMap = getPortalUsersDtosMap();
    var caseEventView = withdrawalCaseEventService.getCaseEventViewForApplicationWithdrawal(
        firstWithdrawalRequestedEvent, portalUsersDtoMap);

    assertThat(caseEventView)
        .usingRecursiveComparison()
        .isEqualTo(
            new CaseEventView(
                APPLICATION_WITHDRAWAL_REQUESTED.getCaseEventHeader(),
                APPLICATION_WITHDRAWAL_REQUESTED.getCaseEventUserLabel(),
                portalUsersDtoMap.get(firstWithdrawalRequestedEvent.mainEventUserWuaId()).displayName(),
                null,
                null,
                APPLICATION_WITHDRAWAL_REQUESTED.getCaseEventDateTimeLabel(),
                DateUtils.format(firstWithdrawalRequestedEvent.eventDateTime(), DateUtils.DATE_TIME),
                String.valueOf(firstWithdrawalRequestedEvent.applicationVersion().getVersion()),
                APPLICATION_WITHDRAWAL_REQUESTED.getCaseEventTextLabel(),
                firstWithdrawalRequestedEvent.eventText(),
                null
            )
        );
  }
  ...
  @Test
  void getCaseEventViewForApplicationWithdrawal_onResponse() {
    var portalUsersDtoMap = getPortalUsersDtosMap();
    var caseEventView = withdrawalCaseEventService.getCaseEventViewForApplicationWithdrawal(
        firstWithdrawalRespondedEvent, portalUsersDtoMap);

    assertThat(caseEventView)
        .usingRecursiveComparison()
        .isEqualTo(
            new CaseEventView(
                APPLICATION_WITHDRAWAL_RESPONDED.getCaseEventHeader(),
                APPLICATION_WITHDRAWAL_RESPONDED.getCaseEventUserLabel(),
                portalUsersDtoMap.get(firstWithdrawalRespondedEvent.mainEventUserWuaId()).displayName(),
                null,
                null,
                APPLICATION_WITHDRAWAL_RESPONDED.getCaseEventDateTimeLabel(),
                DateUtils.format(firstWithdrawalRespondedEvent.eventDateTime(), DateUtils.DATE_TIME),
                String.valueOf(firstWithdrawalRespondedEvent.applicationVersion().getVersion()),
                APPLICATION_WITHDRAWAL_RESPONDED.getCaseEventTextLabel(),
                firstWithdrawalRespondedEvent.eventText(),
                null
            )
        );
  }
