...
@ContextConfiguration(classes = {
    OpredAddRolesController.class,
    OpredTeamMemberRolesValidator.class
})
class OpredAddRolesControllerTest extends AbstractControllerTest {
  ...
  void renderAddTeamMemberRoles_whenUserIsNotLoggedIn_thenRedirectedToLogin() throws Exception {
    ...
    when(teamService.getTeam(TEAM_ID, OpredAddRolesController.TEAM_TYPE)).thenReturn(Optional.of(TEAM));
    ...
    mockMvc.perform(get(ReverseRouter.route(on(OpredAddRolesController.class).renderAddTeamMemberRoles(TEAM_ID, webUserAccountIdToAdd))))
  ...
  void renderAddTeamMemberRoles_whenAccessManager_thenOk() throws Exception {
    ...
    when(energyPortalUserService.findByWuaId(teamMember.wuaId())).thenReturn(Optional.of(energyPortalUser));

    mockMvc.perform(get(ReverseRouter.route(on(OpredAddRolesController.class)
            .renderAddTeamMemberRoles(TEAM_ID, teamMember.wuaId())))
            .with(user(user)))
  ...
  void renderAddTeamMemberRoles_whenNoTeamFound_thenNotFound() throws Exception {
    ...
    when(teamService.getTeam(TEAM_ID, OpredAddRolesController.TEAM_TYPE)).thenReturn(Optional.empty());
  ...
  void renderAddTeamMemberRoles_whenNoEnergyPortalUser_thenNotFound() throws Exception {
    ...
    when(teamMemberService.isMemberOfTeam(TEAM_ID, user)).thenReturn(true);
    when(teamService.getTeam(TEAM_ID, OpredAddRolesController.TEAM_TYPE)).thenReturn(Optional.of(TEAM));
    when(energyPortalUserService.findByWuaId(teamMember.wuaId())).thenReturn(Optional.empty());
  ...
  void renderAddTeamMemberRoles_assertModelProperties() throws Exception {
    ...
    mockMvc.perform(get(ReverseRouter.route(on(OpredAddRolesController.class)
        ...
        .andExpect(model().attribute("pageTitle", "What actions does %s perform?".formatted(energyPortalUser.displayName())))
        .andExpect(model().attribute("backLinkUrl", ReverseRouter.route(on(OpredAddMemberController.class).renderAddTeamMember(TEAM_ID))))
        .andExpect(model().attributeExists("form"))
        .andExpect(model().attribute("roles", getDisplayableOpredRoles()));
  }
  ...
  void saveAddTeamMemberRoles_whenUserIsNotLoggedIn_thenRedirectedToLogin() throws Exception {
    ...
    mockMvc.perform(post(ReverseRouter.route(on(OpredAddRolesController.class)
            .saveAddTeamMemberRoles(TEAM_ID, webUserAccountIdToAdd, null, null)))
            .with(csrf()))
        .andExpect(redirectionToLoginUrl());
  }
  ...
  void saveAddTeamMemberRoles_whenAccessManager_thenOk() throws Exception {
    ...
    mockMvc.perform(get(ReverseRouter.route(on(OpredAddRolesController.class)
            .saveAddTeamMemberRoles(TEAM_ID, teamMember.wuaId(), null, null)))
            .with(user(user)))
        .andExpect(status().isOk());
  }
  ...
  void saveAddTeamMemberRoles_whenValidTeamMemberRolesForm_thenRedirectionToMembersPage() throws Exception {
    ...
    mockMvc.perform(post(ReverseRouter.route(on(OpredAddRolesController.class)
            ...
            .with(user(user))
            .with(csrf())
            .param("roles", OpredTeamRole.ACCESS_MANAGER.name()))
        .andExpect(status().is3xxRedirection())
        .andExpect(redirectedUrl(ReverseRouter.route(on(OpredTeamManagementController.class).renderMemberList(TEAM_ID))));
  }

  private Map<String, String> getDisplayableOpredRoles() {
    return Arrays.stream(OpredTeamRole.values())
        .sorted(Comparator.comparing(OpredTeamRole::getDisplayOrder))
  ...
  private static Stream<Arguments> getEnergyPortalUserThatShouldResultInBadRequest() {
    var noLoginEnergyPortalUser = EnergyPortalUserDtoTestUtil.Builder().canLogin(false).build();
    var sharedAccountEnergyPortalUser = EnergyPortalUserDtoTestUtil.Builder().hasSharedAccount(true).build();

    return Stream.of(Arguments.of(noLoginEnergyPortalUser), Arguments.of(sharedAccountEnergyPortalUser));
  }
}
