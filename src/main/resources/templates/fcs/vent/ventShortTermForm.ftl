...
<@defaultPage htmlTitle=pageTitle pageHeading=pageTitle errorItems=errorList pageSize=PageSize.FULL_WIDTH>
  <@grid.gridRow>
    <@grid.twoThirdsColumn>
      <p class="govuk-body-l">
        Enter the amount of venting expected for each month for the requested consent days only. For example, if the consent is due to end halfway through the month, only provide vent volumes for half the month.
      </p>
  ...
  <@fdsForm.htmlForm actionUrl=springUrl(submitUrl)>
    <table class="govuk-table" data-module="fcs-table-with-pastable-content">
      ...
      <#list form.ventShortTermMonthForms as monthForm>
        <tr class="govuk-table__row">
            <#assign currentMonthForm = "form.ventShortTermMonthForms[${monthForm_index}]"/>
