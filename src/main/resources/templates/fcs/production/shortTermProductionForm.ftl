      ...
      <table class="govuk-table" data-module="fcs-table-with-pastable-content">
        ...
        <#list form.shortTermProductionMonthForms as monthForm>
          <tr class="govuk-table__row">
            ...
            <input type="hidden" name="${spring.status.expression}" value="${spring.stringStatusValue}">
            <#assign monthAndYear = "${monthForm.month[0..2]} ${monthForm.year}" />
            <td class="govuk-table__cell">${monthAndYear}</td>
            <td class="govuk-table__cell">${monthForm.consentDays}</td>
            <td class="govuk-table__cell">
                <@fdsTextInput.textInput path="form.shortTermProductionMonthForms[${monthForm_index}].oilMinValue.inputValue" labelText="Oil min value ${monthAndYear}" labelClass="govuk-visually-hidden"/>
            </td>
            <td class="govuk-table__cell">
                <@fdsTextInput.textInput path="form.shortTermProductionMonthForms[${monthForm_index}].oilMaxValue.inputValue" labelText="Oil max value ${monthAndYear}" labelClass="govuk-visually-hidden"/>
            </td>
            <td class="govuk-table__cell">
                <@fdsTextInput.textInput path="form.shortTermProductionMonthForms[${monthForm_index}].gasMinValue.inputValue" labelText="Gas min value ${monthAndYear}" labelClass="govuk-visually-hidden"/>
            </td>
            <td class="govuk-table__cell">
                <@fdsTextInput.textInput path="form.shortTermProductionMonthForms[${monthForm_index}].gasMaxValue.inputValue" labelText="Gas max value ${monthAndYear}" labelClass="govuk-visually-hidden"/>
            </td>
          </tr>
        </#list>
        </tbody>
