...
public class SignaturePlaceholderLocator extends PDFTextStripper {
  ...
  protected void writeString(String text, List<TextPosition> textPositions) {
    if (text.trim().equals(DigitalSignatureMailMergeField.SIGNATURE_PLACEHOLDER_TEXT)) {
      if (placeholderFound) {
        throw new RuntimeException("Multiple digital signature placeholders found in document");
      }
  ...
  public FtssVisualSignatureProperties.SignatureCoordinates getSignaturePlaceholderLocation(PDDocument document) {
    try {
      ...
      if (!placeholderFound) {
        throw new RuntimeException("No signature placeholder found in document");
      }
      ...
      throw new RuntimeException("Failed to parse signature placeholder", e);
    }
  }
}
